...
import { InfoResponse as CwProposalSingleInfoResponse } from '@dao-dao/state/clients/cw-proposal-single'

import {
  CW20STAKEDBALANCEVOTING_CONTRACT_NAME,
  CW4VOTING_CONTRACT_NAME,
  CWPROPOSALMULTIPLE_CONTRACT_NAME,
...
export const parseVotingModuleContractName = (
  contractName: string
): VotingModuleType | undefined =>
...
export const parseProposalModuleContractName = (
  contractName: string
): ProposalModuleType | undefined =>
  contractName.includes(CWPROPOSALSINGLE_CONTRACT_NAME)
    ? ProposalModuleType.CwProposalSingle
    : contractName.includes(CWPROPOSALMULTIPLE_CONTRACT_NAME)
    ? ProposalModuleType.CwProposalMultiple
    : undefined
  ...
  const getV0_1_0ProposalModules = async () =>
    (
      ...
      .map(async (address, index) => {
        const {
          info: { contract: proposalModuleContractName },
        }: CwProposalSingleInfoResponse = await cwClient.queryContractSmart(
          address,
          {
            info: {},
          }
        ...
        const type = parseProposalModuleContractName(proposalModuleContractName)
        if (!type) {
          throw new Error('Failed to determine proposal module type.')
        }
