...
export class UpdateDelegationConfigAction extends ActionBase<UpdateDelegationConfigData> {
  ...
  constructor(
    ...
    /**
     * Skip loading widget from DAO by providing the address directly.
     */
    ...
    if (voteDelegationAddress) {
      ...
      const voteDelegation =
        options.context.dao.getWidget<VoteDelegationWidgetData>(
          WidgetId.VoteDelegation
        )?.values
      if (!voteDelegation?.address) {
        throw new Error('Vote delegation widget not found.')
      }
