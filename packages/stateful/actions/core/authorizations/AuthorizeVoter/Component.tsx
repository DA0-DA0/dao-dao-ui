...
export type AuthorizeVoterData = {
  chainId: string
  voter: string
...
export type AuthorizeVoterOptions = {
  AddressInput: ComponentType<AddressInputProps<AuthorizeVoterData>>
}

export const AuthorizeVoterComponent: ActionComponent<
  AuthorizeVoterOptions
> = ({ fieldNamePrefix, errors, isCreating, options: { AddressInput } }) => {
  const { t } = useTranslation()
  const { bech32_prefix: bech32Prefix } = useChain()

  const { register } = useFormContext<AuthorizeVoterData>()
